...
def load_jobshop(number_total_jobs, number_total_machines, data, JobShop):
...
def parse(JobShops, instance, from_absolute_path=False, load_deterministic=False):
    ...
    with open(data_path, "r") as data:
        ...
        if data[0] == '\n':
            data.pop(0)
        # First instances in file is the deterministic data so skip this.
        if not load_deterministic:
            for key, line in enumerate(data):
                if key >= number_total_jobs:
                    break
            data = data[number_total_jobs:]
        elif load_deterministic:
            det_JobShop = JobShop()
            data = load_jobshop(number_total_jobs, number_total_machines, data, det_JobShop)
            

        for shop in JobShops:
            data = load_jobshop(number_total_jobs, number_total_machines, data, shop)

    return (det_JobShop, JobShops) if load_deterministic else JobShops
