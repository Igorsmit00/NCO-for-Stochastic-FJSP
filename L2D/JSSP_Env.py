...
class SJSSP(gym.Env, EzPickle):
    def __init__(self, n_j, n_m):
        ...
        self.first_col = np.arange(start=0, stop=self.number_of_tasks, step=1).reshape(
            self.number_of_jobs, -1
        )[:, 0]
        # the task id for last column
        self.last_col = np.arange(start=0, stop=self.number_of_tasks, step=1).reshape(
            self.number_of_jobs, -1
        )[:, -1]
    ...
    def step(self, action):
        ...
        fea = np.concatenate(
            (
                self.LBs.reshape(self.num_rea + 1, -1, 1) / configs.et_normalize_coef,
                self.finished_mark.reshape(self.num_rea + 1, -1, 1),
            ),
            axis=-1,
        )
        reward = -(
            np.quantile(
                self.LBs.reshape((self.num_rea + 1, -1))[101:].max(axis=-1), q=0.95
            )
            - np.quantile(self.max_endTime[101:], q=0.95)
        )
        if reward == 0:
            reward = configs.rewardscale
            self.posRewards += reward
        self.max_endTime = self.LBs.reshape((self.num_rea + 1, -1)).max(axis=-1)

        return self.adj, fea, reward, self.done(), self.omega, self.mask

    @override
    def reset(self, data, data_stoch=None):

        self.step_count = 0
        self.num_rea = data_stoch.shape[0]
        self.m = np.concatenate((np.expand_dims(data[-1], axis=0), data_stoch), axis=0)
        self.m = data[-1]
        # self.dur = data[0].astype(np.single)
        self.dur = np.concatenate(
            (np.expand_dims(data[0], axis=0), data_stoch), axis=0
        ).astype(np.single)
        ...
        self.initQuality = (
            self.LBs.reshape((self.num_rea + 1, -1)).max(axis=-1)
            if not configs.init_quality_flag
            else 0
        )
        self.max_endTime = self.initQuality
        self.finished_mark = np.zeros_like(self.LBs, dtype=np.single)
        ...
        self.mchsStartTimes = -configs.high * np.ones_like(
            np.transpose(self.dur, axes=(0, 2, 1)), dtype=np.int32
        )
        # Ops ID on machines
        self.opIDsOnMchs = -self.number_of_jobs * np.ones_like(
            np.transpose(self.dur, axes=(0, 2, 1)), dtype=np.int32
        )
