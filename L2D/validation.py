def validate(vali_set, vali_data_stoch, model):
    ...
    for i, data in enumerate(vali_set):
        adj, fea, candidate, mask = env.reset(data, vali_data_stoch[i])
        rewards = -np.quantile(env.initQuality[101:], 0.95)
        while True:
            fea_tensor = torch.from_numpy(np.copy(fea[:100])).to(device)
...
if __name__ == "__main__":
    ...
    for SEED in SEEDs:
        ...
        vali_data = [
            uni_instance_gen(n_j=N_JOBS_P, n_m=N_MACHINES_P, low=LOW, high=HIGH)
            for _ in range(params.n_vali)
        ]
