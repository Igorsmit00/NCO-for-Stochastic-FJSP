...
from generate_random_instances import create_stochastic_realizations_random_variances
from mb_agg import *
from models.actor_critic import ActorCritic, ActorCriticSPM
    ...
    def update(self, memories, n_tasks, g_pool):
        ...
        for i in range(len(memories)):
            ...
            for reward, is_terminal in zip(
                reversed(memories[i].r_mb), reversed(memories[i].done_mb)
            ):
                if is_terminal:
                    discounted_reward = 0
            ...
            fea_mb_t = torch.stack(memories[i].fea_mb).to(device).transpose(0, 1)
            fea_mb_t = fea_mb_t.reshape(fea_mb_t.size(0), -1, fea_mb_t.size(-1))
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(len(memories)):
                ...
                loss = (
                    vloss_coef * v_loss + ploss_coef * p_loss + entloss_coef * ent_loss
                )
                loss_sum += loss
                vloss_sum += v_loss
...
def main():
    ...
    ppo = PPO(
        ...
        stoch_attention=configs.SAA_attention,
        scenario_dim=configs.SAA_dim,
    )
    ...
    optimal_gap = 1
    record = 100000
    for i_update in tqdm(range(configs.max_updates)):
        ...
        for i, env in enumerate(envs):
            data = data_generator(
                n_j=configs.n_j, n_m=configs.n_m, low=configs.low, high=configs.high
            )
            data_stoch = create_stochastic_realizations_random_variances(data, 1100)
            adj, fea, candidate, mask = env.reset(data, data_stoch)  # [:100])
            # rew_env.reset(data, data_stoch[100:])
            adj_envs.append(adj)
            fea_envs.append(fea[:100])
            candidate_envs.append(candidate)
            mask_envs.append(mask)
            ep_rewards[i] = -np.quantile(env.initQuality[101:], 0.95)
        ...
        while True:
            ...
            mask_tensor_envs = [
                torch.from_numpy(np.copy(mask)).to(device) for mask in mask_envs
            ]
            ...
            for i in range(configs.num_envs):
                ...
                # rew_envs[i].step(action_envs[i].item())
                adj_envs.append(adj)
                fea_envs.append(fea[:100])
                candidate_envs.append(candidate)
                mask_envs.append(mask)
        ...
        loss, v_loss = ppo.update(
            memories, configs.n_j * configs.n_m, configs.graph_pool_type
        )
        ...
        if (i_update + 1) % 2 == 0:
            file_writing_obj = open(
                ...
                + "_"
                + str(configs.high)
                + f"{'attn' if configs.SAA_attention else ''}"
                + ".txt",
                "w",
            )
        ...
        # log results
        print(
            "Episode {}\t Last reward: {:.2f}\t Mean_Vloss: {:.8f}".format(
        ...
        t4 = time.time()
        if (i_update + 1) % 2 == 0:
            vali_result = -validate(vali_data, vali_data_stoch, ppo.policy).mean()
            validation_log.append(vali_result)
            if vali_result < record:
                torch.save(
                    ppo.policy.state_dict(),
                    "./SavedNetwork/{}.pth".format(
                        "stoch"
                        + str(configs.n_j)
                        + "_"
                        + str(configs.n_m)
