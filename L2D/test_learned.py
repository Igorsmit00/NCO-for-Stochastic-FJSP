...
from mb_agg import *
from Params import configs, str2bool

device = torch.device(configs.device)

parser = argparse.ArgumentParser(description="Arguments for ppo_jssp")
parser.add_argument(
    "--Pn_j", type=int, default=20, help="Number of jobs of instances to test"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str2bool,
    default=False,
    help="Flag of whether use deterministic model",
)
...
stoch_model_str = "stoch" if not params.det_model else ""
attn_str = "attn" if configs.SAA_attention and not params.det_model else ""
...
ppo.policy.load_state_dict(torch.load(path, map_location=torch.device("cuda")))
# ppo.policy.eval()
...
def test(dataset):
    ...
    for i, data in enumerate(dataset):
        adj, fea, candidate, mask = env.reset(data, data_stoch[i])
        ep_reward = -np.quantile(env.max_endTime[101:], 0.95)
        # delta_t = []
        # t5 = time.time()
        while True:
            # t3 = time.time()
            fea_tensor = torch.from_numpy(fea[:100]).to(device)
