...
# args for data load
parser.add_argument('--model_source', type=str, default='SD3', help='Suffix of the data that model trained on')
...
parser.add_argument('--data_type', type=str, default="vali", help='Generated data type (test/vali)')
...
# args for seed
parser.add_argument('--seed_datagen', type=int, default=200, help='Seed for data generation') # 200
parser.add_argument('--seed_train_vali_datagen', type=int, default=100, help='Seed for generate validation data')
parser.add_argument('--seed_train', type=int, default=400, help='Seed for training') # 300
...
# args for network
parser.add_argument('--fea_j_input_dim', type=int, default=10, help='Dimension of operation raw feature vectors') # 10
parser.add_argument('--fea_m_input_dim', type=int, default=8, help='Dimension of machine raw feature vectors') # 8
...
parser.add_argument('--layer_fea_output_dim', nargs='+', type=int, default=[32, 8], # [32, 8]
                    help='Output dimension of the DAN layers')
parser.add_argument('--SAA_attention', type=str2bool, default=False, help='Whether using SAA attention (True) or regular attention (False)')
parser.add_argument('--SAA_attention_dim', type=int, default=32, help='Dimension of SAA attention') # 16

parser.add_argument('--num_mlp_layers_actor', type=int, default=3, help='Number of layers in Actor network')
parser.add_argument('--hidden_dim_actor', type=int, default=64, help='Hidden dimension of Actor network') # 128
...
parser.add_argument('--lr', type=float, default=3e-4, help='Learning rate') #3e-4
...
parser.add_argument('--minibatch_size', type=int, default=143, help='Batch size for computing the gradient')
parser.add_argument('--gradient_accumulation_steps', type=int, default=7, help='Number of gradient accumulation steps')
parser.add_argument('--train_step_type', type=str, default='SAA', help='Type of training step (random/SAA)')
parser.add_argument('--variance', type=str, default='random', help='Type of variance (fixed/random)')
parser.add_argument('--variance_dist', type=str, default='lognormal', help='Type of variance distribution (lognormal/beta/be_lo/be_lo_ga)')
parser.add_argument('--num_eval_realizations', type=int, default=1000, help='Number of realizations for evaluation')
parser.add_argument('--num_input_realizations', type=int, default=100, help='Number of realizations for input into network')
...
parser.add_argument('--test_model', nargs='+', default=['random20x10+mix_stoch_attn_SAA_mean'], help='List of model for testing')
parser.add_argument('--test_method', nargs='+', default=[], help='List of heuristic methods for testing')
parser.add_argument('--deter_model', type=str2bool, default=False, help='Whether using deterministic model in testing')

parser.add_argument('--stoch_obj', type=str, default="VaR", help='Objective of stochastic model (mean/VaR)')
parser.add_argument('--VaR_alpha', type=float, default=0.95, help='Confidence level of VaR')
parser.add_argument('--stoch_sample_each_step', type=str2bool, default=False, help='Whether sampling each step in testing')
parser.add_argument('--scenario-agg', type=str, default='mean', help='Aggregation method of scenarios after attention(mean/PMA)')
