...
class Memory:
    ...
    def to_cpu(self):
        self.fea_j_seq = [item.cpu() for item in self.fea_j_seq]
        self.op_mask_seq = [item.cpu() for item in self.op_mask_seq]
        self.fea_m_seq = [item.cpu() for item in self.fea_m_seq]
        self.mch_mask_seq = [item.cpu() for item in self.mch_mask_seq]
        self.dynamic_pair_mask_seq = [item.cpu() for item in self.dynamic_pair_mask_seq]
        self.comp_idx_seq = [item.cpu() for item in self.comp_idx_seq]
        self.candidate_seq = [item.cpu() for item in self.candidate_seq]
        self.fea_pairs_seq = [item.cpu() for item in self.fea_pairs_seq]
        self.action_seq = [item.cpu() for item in self.action_seq]
        self.reward_seq = [item.cpu() for item in self.reward_seq]
        self.val_seq = [item.cpu() for item in self.val_seq]
        self.done_seq = [item.cpu() for item in self.done_seq]
        self.log_probs = [item.cpu() for item in self.log_probs]

    def to_cuda(self):
        self.fea_j_seq = [item.cuda() for item in self.fea_j_seq]
        self.op_mask_seq = [item.cuda() for item in self.op_mask_seq]
        self.fea_m_seq = [item.cuda() for item in self.fea_m_seq]
        self.mch_mask_seq = [item.cuda() for item in self.mch_mask_seq]
        self.dynamic_pair_mask_seq = [
            item.cuda() for item in self.dynamic_pair_mask_seq
        ]
        self.comp_idx_seq = [item.cuda() for item in self.comp_idx_seq]
        self.candidate_seq = [item.cuda() for item in self.candidate_seq]
        self.fea_pairs_seq = [item.cuda() for item in self.fea_pairs_seq]
        self.action_seq = [item.cuda() for item in self.action_seq]
        self.reward_seq = [item.cuda() for item in self.reward_seq]
        self.val_seq = [item.cuda() for item in self.val_seq]
        self.done_seq = [item.cuda() for item in self.done_seq]
        self.log_probs = [item.cuda() for item in self.log_probs]
...
class PPO:
    def __init__(self, config):
        ...
        if config.SAA_attention:
            self.policy = SPM_DAN(config)
            # for param in self.policy.fea_j_embedder.parameters():
            #     param.requires_grad = False
            # for param in self.policy.fea_m_embedder.parameters():
            #     param.requires_grad = False
        ...
        if isinstance(self.policy, SPM_DAN):
            # self.optimizer = torch.optim.Adam(
            #     [{"params": list(set(self.policy.parameters()) - set(self.policy.fea_pair_embedder.parameters()))},
            #     {"params": self.policy.fea_pair_embedder.parameters(), "lr": self.lr}], lr=self.lr)
        ...
        # self.lr_scheduler = torchtune.modules.get_cosine_schedule_with_warmup(self.optimizer, num_warmup_steps=50, num_training_steps=1000)
            ...
            for i in range(int(num_batch)):
                ...
                if isinstance(self.policy, (SPM_DAN)):
                    pis, vals = self.policy(
                        fea_j=t_data[0][start_idx:end_idx].to(self.device),
                        op_mask=t_data[1][start_idx:end_idx].to(self.device),
                        candidate=t_data[6][start_idx:end_idx].to(self.device),
                        fea_m=t_data[2][start_idx:end_idx].to(self.device),
                        mch_mask=t_data[3][start_idx:end_idx].to(self.device),
                        comp_idx=t_data[5][start_idx:end_idx].to(self.device),
                        dynamic_pair_mask=t_data[4][start_idx:end_idx].to(self.device),
                        fea_pairs=t_data[7][start_idx:end_idx].to(self.device),
                        num_samples=100,
                    )
                else:
                    pis, vals = self.policy(
                ...
                action_batch = t_data[8][start_idx:end_idx].to(self.device)
                logprobs, ent_loss = eval_actions(pis, action_batch)
                ratios = torch.exp(
                    logprobs - t_data[12][start_idx:end_idx].to(self.device).detach()
                )

                advantages = t_advantage_seq[start_idx:end_idx].to(self.device)
                ...
                v_loss = self.V_loss_2(
                    vals.squeeze(1), v_target_seq[start_idx:end_idx].to(self.device)
                )
